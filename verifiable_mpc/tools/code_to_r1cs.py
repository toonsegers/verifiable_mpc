...
def flatten_expr(target, expr):
    ...
    # Or, for that matter, x = y (op) 5
        ...
        # If the subexpression is a variable or a number, then include it
        # directly
        if isinstance(expr.left, (ast.Name, ast.Num)):
            var1 = expr.left.id if isinstance(
                expr.left, ast.Name) else expr.left.n
            sub1 = []
        ...
        if isinstance(expr.right, (ast.Name, ast.Num)):
            var2 = expr.right.id if isinstance(
                expr.right, ast.Name) else expr.right.n
            sub2 = []
        else:
            var2 = mksymbol()
...
def assign_variables(inputs, input_vars, flatcode):
    ...
    for x in flatcode:
        if x[0] == 'set':
            ...
            grab_var(varz, assignment, x[2]) + grab_var(varz, assignment, x[3])
        elif x[0] == '-':
            assignment[varz.index(x[1])] = \
            grab_var(varz, assignment, x[2]) - grab_var(varz, assignment, x[3])
        elif x[0] == '*':
